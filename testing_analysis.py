...
def main():
    ...
    modules = list(TARGET_SWEEPS.keys())

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    analysis_out = base_dir / "test_analysis" / f"sweep_summary_{timestamp}"
    ...
    for mod, target_sweep in TARGET_SWEEPS.items():
        if not target_sweep.exists():
            print(f"  Skipping module '{mod}' (Target sweep not found: {target_sweep})")
            continue
    ...
    for mod in modules:
        ...
        for idx, ekey in enumerate(energy_keys):
            ...
            # Direct mapping from hardcoded path
            edir_path = TARGET_SWEEPS[mod] / ekey
