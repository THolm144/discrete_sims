...
# ── NEW CHERENKOV TOGGLE ─────────────────────────────────────────────────────
DISABLE_CHERENKOV="true"   # "true" to kill Cherenkov while keeping scintillation, "false" for both
# ─────────────────────────────────────────────────────────────────────────────
...
echo "Optical Physics: ${OPTICAL} (Disable Cherenkov: ${DISABLE_CHERENKOV})"

for i in $(seq 0 $((N_RUNS - 1))); do
    echo "  Run ${i}..."
    
    # Build command arguments dynamically based on toggle
    CMD_ARGS=(
        --world       "$WORLD"
        --particle    "$PARTICLE"
    ...
    if [ "$DISABLE_CHERENKOV" = "true" ]; then
        CMD_ARGS+=(--disable-cherenkov)
    fi

    python3 simulator.py "${CMD_ARGS[@]}"
done
