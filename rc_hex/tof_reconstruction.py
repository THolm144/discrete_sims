...
def main():
    ...
    # 1. Broad recursive glob to catch files independent of how deep they are nested
    ...
    # 2. Reconstruct unique parent run directories to feed the truth loader safely
    ...
    for fpath in hit_files:
        ...
        for eid, ti in zip(ev[mask_e_up], gt[mask_e_up]):
            key = int(eid)  # Strip run_tag to prevent structural coincidence mismatch
