...
# 3. METHODOLOGY: WAVEFORM GENERATION (Bridging Geant4 to Analysis)
# ==============================================================================
def simulate_sipm_waveform(photon_times_ns, time_step_ps=10, max_time_ps=5000, yield_scale_factor=10):
    """
    Converts discrete Geant4 photon arrival times into a digitized waveform array.
    yield_scale_factor: Used to scale up hits if Geant4 was run at a reduced Light Yield.
    """
    # 1. Convert Geant4 nanoseconds to picoseconds
    times_ps = np.array(photon_times_ns) * 1000 
    
    # 2. Add realistic SiPM jitter (SPTR ~150ps) to the raw photons BEFORE histogramming
    times_ps += np.random.normal(0, 150, size=len(times_ps))
    
    # 3. Create time bins (like digitizer samples)
    bins = np.arange(0, max_time_ps + time_step_ps, time_step_ps)
    
    # 4. Histogram the hits. This gives us the raw pulse shape from the 10% yield run
    pulse_shape, _ = np.histogram(times_ps, bins=bins)
    
    # ---------------------------------------------------------
    # 5. THE MAGIC SCALING STEP
        ...
        for root_file in hit_files:
            try:
                ...
                with uproot.open(root_file) as f:
                    tree_key = f.keys()[0]  # Dynamically gets 'detector_hits_X;1'
                    tree = f[tree_key]
