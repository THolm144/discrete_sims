...
=====================================
Optimized version of unified_sweep_analysis_fixed.py.

Aggregates timing-resolution, ToF-reconstruction, and energy linearity results
across all four RADiCAL geometry variants (radi_cal_energy, radi_cal_triple,
rc_hex, rc_hex_triple) and all energy points in each module's latest sweep.

Fun Facts / Notes
-----------------------------

1. Per-event aggregation (`up_first`, `down_first`, BestMinus quantiles,
   E/T hit counts) is now done with vectorized pandas groupby operations
   instead of pure-Python `for eid, ti in zip(...)` loops. This is the
   single biggest win — those loops were O(n_photons) in the interpreter.
2. Each ROOT file's branches are read in one `tree.arrays([...])` call
   instead of seven separate `.array()` calls.
3. `run_dirs` (the list of run directories under each energy point) is
   computed once inside `analyze_energy_batch` and cached on the result
   dict, instead of being re-derived via `rglob` three separate times
   during plotting.
4. The independent (module, energy) analysis jobs are farmed out to a
   `ProcessPoolExecutor` since they share no state.
5. `master_summary` is pickled to disk after the analysis pass, and can be
   reloaded with `--from-cache <path>` to iterate on plotting code without
   re-parsing ROOT files every time.
...
Outputs (in analysis/sweep_summary_<timestamp>/):
    ...
    {module}_energy_performance.png
    timing_resolution_vs_energy.png
    timing_vs_energy_report.txt
    master_summary_<timestamp>.pkl   (for --from-cache re-use)
"""
import argparse
import datetime
import pickle
...
def _chunk_series(mask, values, ev, run_tag):
    """Build a (run_tag, EventID)-multi-indexed Series for the masked rows.
    Returns None if the mask selects nothing, so callers can skip empty chunks
    cheaply instead of concatenating a pile of zero-length Series later."""
    n = int(mask.sum())
    if n == 0:
        return None
    idx = pd.MultiIndex.from_arrays(
        [np.full(n, run_tag, dtype=object), ev[mask].astype(np.int64)]
    )
    return pd.Series(values[mask], index=idx)


def _grouped(chunks, how):
    """Concatenate per-file chunks and reduce once with a single groupby call.
    `how` is 'min', 'count', or a quantile float in [0, 1]."""
    if not chunks:
        return {}
    s = pd.concat(chunks)
...
def analyze_energy_batch(batch_dir: Path, is_hex: bool,  module_name: str, verbose_label: str = ""):
    ...
    # Per-file chunks; reduced once at the end instead of per-row Python loops.
    ...
    branch_list = ["Position_X", "Position_Y", "Position_Z", "GlobalTime",
                   "LocalTime", "EventID", "ParticleName"]
    ...
    for fpath in hit_files:
        ...
        try:
            with uproot.open(fpath) as f:
                ...
                if tree.num_entries == 0:
                    continue
                arrs = tree.arrays(branch_list, library="np")  # single read, not 7
        except Exception:
            continue
        ...
        if c is not None:
            ...
            dw_e_hit_chunks.append(c)  # index reused for counting; values ignored
    ...
    # ── one groupby reduction per quantity, across all files at once ──
    ...
    # ── BestMinus timing (T-type coincidence) ──
    ...
    return {
        ...
        "run_dirs": sorted(run_dirs),  # cached so plotting never re-globs
    }
...
def main():
    ...
    parser.add_argument("--from-cache", type=str, default=None,
                         help="Path to a previously pickled master_summary; "
                              "skips ROOT parsing and jumps straight to plotting.")
    ...
    if args.from_cache:
        ...
        # Independent (module, energy) jobs -> farm out across processes.
        import os
        n_workers = min(100, max(1, (os.cpu_count() or 2) - 1))
        print(f"Running {len(jobs)} analysis jobs across {n_workers} worker process(es)...\n")
        ...
        print(f"\nCached raw results to {cache_path} (reuse with --from-cache for fast re-plotting)\n")
    ...
    for mod in modules:
        ...
        for idx, ekey in enumerate(energy_keys):
            ...
            if len(first_times) > 0:
                ...
                ax.set_xlabel("Kinematic Pseudo-Distance (mm)", fontsize=9)
        ...
        # ─────────────────────────────────────────────────────────────────────
        # 3.7. ENERGY LINEARITY AND RESOLUTION PANELS
        # ─────────────────────────────────────────────────────────────────────
